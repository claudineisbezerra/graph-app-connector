...
urlFragment: graph-connector-tutorial
...
| `.../GraphConnectorApplication.java`                                    | Main class.                                                                                 |
...
| `.../TeamsUtilities.java`                                                                | Utility Class (e.g., filter ID token claims, MS Graph SDK initialization and use).          |
...
```console
    git clone https://github.com/XXXXX/graph-connector-tutorial.git
    cd graph-connector-tutorial
    cd graphconnector
```
...
To extract token details, make use of Spring Security's `AuthenticationPrincipal` and `OidcUser` object in a request mapping. See the [Sample Controller](src/main/java/com/ebao/graphconnector/SampleController.java) for an example of this app making use of ID Token claims.
...
public String callGraph(@RegisteredOAuth2AuthorizedClient("graph") OAuth2AuthorizedClient graphAuthorizedClient) {
  ...
  GraphServiceClient graphServiceClient = TeamsUtilities.getGraphServiceClient(graphAuthorizedClient);
